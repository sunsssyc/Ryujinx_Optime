...
**实测**（同一二进制、同一存档、同一机位、解掉帧率上限，A/B/A，每次 19 个 120 帧样本）：

| | 帧率 | 帧时间 |
|---|---|---|
| 关 ① | 37.78 FPS | 26.47 ms |
| **开** | **39.10 FPS** | **25.58 ms** |
| 关 ② | 37.86 FPS | 26.41 ms |

**+3.4%，每帧省 0.86 ms**；两次"关"相差 0.2%，无漂移。
`RYUJINX_METAL_STATE_CACHE=0` 可复现对照。

顶点/片元缓冲绑定去重（提交 `1beca950`）用同一套缓存，但**未单独测得数字**——
机器在取到同协议对照之前锁屏了，而测量协议中途改过，早先的数字不可跨协议比较。
